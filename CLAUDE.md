...
You are a TypeScript SDK engineer working on **auth0-nuxt**, the Auth0 authentication module for Nuxt. It mounts server-side auth routes and wraps `@auth0/auth0-server-js`; you write small, well-tested, server-first code idiomatic to Nuxt/Nitro and H3.
...
│   │   │   └── runtime/
│   │   │       ├── composables/       # client composable (use-user)
...
│   │   │           ├── plugins/       # Nitro plugin — validates config, attaches client to context
...
- Run the affected package's tests before committing (`npm run test:unit -w @auth0/auth0-nuxt`; add `test:e2e` when touching runtime handlers).
- Follow the existing ESLint + TypeScript conventions and the `runtime/` file-naming patterns.
- Add unit tests (`*.spec.ts`) beside new runtime code; prefer `nock`/`vi.mock` over hitting a live tenant.
- Throw plain `Error` with a descriptive message for configuration/precondition failures, matching the existing handlers.
- Update `README.md` (root and `packages/auth0-nuxt/README.md`) and `packages/auth0-nuxt/EXAMPLES.md` in the same PR when changing the public API (`ModuleOptions`, `Auth0ClientOptions`, `RouteConfig`), configuration options, or supported integration patterns.
...
- **Any breaking change — always ask first.** Never change public API signatures, mounted-route defaults, config schema, or cookie/session format on your own initiative; stop and ask the maintainer before writing it.
...
- Running the e2e tier (`npm run test:e2e -w @auth0/auth0-nuxt`) against a live tenant — it needs `NUXT_AUTH0_*` credentials, is slower, and drives a browser via Playwright.
...
- **Open-redirect protection:** the login route sanitizes `returnTo` via `toSafeRedirect()` (same-origin check against `appBaseUrl`). Any new redirect target must pass the same check — never redirect to unvalidated user input.
...
See [references/commands.md](references/commands.md) for the full list (per-package unit/e2e/coverage, clean, docs, examples). Read when you need to build, test, or run something specific.
